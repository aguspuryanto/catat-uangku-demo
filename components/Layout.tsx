  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 pb-20 md:pb-0">
      ...
      <aside className="hidden md:flex w-64 bg-white border-r border-slate-200 flex-col sticky top-0 h-screen">
        <div className="p-6 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 p-3 rounded-2xl text-white shadow-lg shadow-indigo-200">
              <Wallet size={24} />
            </div>
            <div>
      ...
      <header className="md:hidden bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 p-2 rounded-xl text-white shadow-lg shadow-indigo-200">
            <Wallet size={20} />
          </div>
          <div>
        ...
        <button 
          onClick={onLogout} 
          className="text-slate-400 hover:text-red-500 hover:bg-red-50 p-2.5 rounded-xl transition-all duration-200"
        >
          <LogOut size={20} />
        </button>
      </header>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-200 px-4 py-2 flex justify-around items-center z-40 shadow-[0_-8px_30px_rgba(0,0,0,0.08)]">
        <button 
          onClick={() => setActiveTab('dashboard')}
          className={`flex flex-col items-center gap-1.5 py-2 px-4 rounded-2xl transition-all duration-200 ${activeTab === 'dashboard' ? 'text-indigo-600 bg-indigo-50 scale-105' : 'text-slate-400 hover:text-slate-600'}`}
        >
        ...
        <button 
          onClick={() => setActiveTab('transactions')}
          className={`flex flex-col items-center gap-1.5 py-2 px-4 rounded-2xl transition-all duration-200 ${activeTab === 'transactions' ? 'text-indigo-600 bg-indigo-50 scale-105' : 'text-slate-400 hover:text-slate-600'}`}
        >
        ...
        <button 
          onClick={() => setActiveTab('pinjaman')}
          className={`flex flex-col items-center gap-1.5 py-2 px-4 rounded-2xl transition-all duration-200 ${activeTab === 'pinjaman' ? 'text-indigo-600 bg-indigo-50 scale-105' : 'text-slate-400 hover:text-slate-600'}`}
        >
